...
实时管线（streaming，**仍在攻关**）复用同一套模型与评测，以 ~1.4-1.7s 中位词延迟增量出草稿、
句末自动精修，L2 档 WER 0.0 / L1 档 0.0845（口径与成色见 §4、§7）。
...
- **当前数字**（K4 口径）：
  ...
  - 延迟（JOURNAL 口径）：草稿轨词延迟中位 a 1.72s / b 1.35s，RTF 0.85/0.88（J4 两遍基线，满足实时）；
     final 轨整句精修出字，为句级延迟——J14 含模板融合定版（即表中 0.0 档）median a 14.7s / b 14.5s，
     RTF 1.06/1.15，慢于实时。
